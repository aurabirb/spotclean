...
| `togglebpmscan`                                                  | Turn the background BPM walk over your library on or off. Bound to <kbd>Shift</kbd>+<kbd>t</kbd> by default. The track you're *playing* is still analyzed either way (its audio is already being downloaded for playback); this only stops the extra CDN traffic from walking the rest of the library. Starting state is set by `bpm_scan_enabled`.                                          |
...
| `bpm_scan_cache_full`           | Have the BPM scanner fetch each track at your configured `bitrate` and download it *in full*, so librespot caches it for playback and offline use. Avoids a second CDN fetch when you play the track (better than the range-only fetch in testing) at the cost of a whole-library download (many GB of CDN traffic); set to `false` to fetch only the analysis window, and raise `bpm_scan_min_interval_secs` if you hit the rate limiter. | `true`, `false` | `true`              |
